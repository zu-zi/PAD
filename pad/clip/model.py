...
class VisionTransformer(nn.Module):
    """CLIP ViT visual encoder with optional per-layer VA-Prompt injection.

    When ``self.va_prompt`` is attached by the outer model, each transformer
    block receives a fresh set of [G-Prompt; E-Prompt(top-k)] tokens inserted
    right after the [CLS] token.  Injected tokens are stripped from the
    residual stream before entering the next block, so the positional layout
    of patch tokens is preserved (cf. PAD Sec. 3.4.2).
    """

    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int,
                 stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        super().__init__()
        ...
        # VA-Prompt is attached by PADModel after construction; leaving these
        # attributes as defaults keeps the pure-CLIP behaviour when the prompt
        # module is absent (e.g. during ablations).
        self.va_strip_after_block = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        cls_tok = self.class_embedding.to(x.dtype).expand(x.shape[0], 1, -1)
        x = torch.cat([cls_tok, x], dim=1)                     # [B, 1 + HW, C]
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x).permute(1, 0, 2)                    # NLD -> LND

        use_va = hasattr(self, "va_prompt") and (self.va_prompt is not None)

        # Cache the penultimate-layer token stream for VISKD (Eq. 7).
        idx_pen = min(10, len(self.transformer.resblocks) - 1)
        x_pen_lnd = None

        for li, block in enumerate(self.transformer.resblocks):
            if use_va:
                x_nld = x.permute(1, 0, 2)                     # [B, L, C]
                out = self.va_prompt.forward_for_layer(li, x_nld, cls_feat=None)
                prompts, strip_len = out["batched"], out["strip_len"]

                # Inject prompts right after the [CLS] token.
                x_cat = torch.cat(
                    [x_nld[:, :1, :], prompts, x_nld[:, 1:, :]], dim=1
                )
                x_after = block(x_cat.permute(1, 0, 2)).permute(1, 0, 2)  # [B, L', C]
                if self.va_strip_after_block:
                    x_after = torch.cat(
                        [x_after[:, :1, :], x_after[:, 1 + strip_len:, :]], dim=1
                    )
                x = x_after.permute(1, 0, 2)
            else:
                x = block(x)
        ...
        x_last_lnd = x
        x_pen = (x_pen_lnd if x_pen_lnd is not None else x_last_lnd).permute(1, 0, 2)
        x_last = self.ln_post(x_last_lnd.permute(1, 0, 2))
        x_proj = x_last @ self.proj if self.proj is not None else x_last
        return x_pen, x_last, x_proj
